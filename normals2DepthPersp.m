...
z_persp = smoothIntegration(vec2Img_o2p(q_tmp,size(mask),mask),vec2Img_o2p(p_tmp,size(mask),mask),mask,grad_option);
z_persp = exp(z_persp);
...
if strcmp(grad_option,'DH')
  [Dxf, Dyf] = getNabla_o2p(mask, 'Forward', 'DirichletHomogeneous');
  [Dxb, Dyb] = getNabla_o2p(mask, 'Backward', 'DirichletHomogeneous');
elseif strcmp(grad_option,'NH')
  [Dxf, Dyf] = getNabla_o2p(mask, 'Forward', 'NeumannHomogeneous');
  [Dxb, Dyb] = getNabla_o2p(mask, 'Backward', 'NeumannHomogeneous');
elseif strcmp(grad_option,'NC')
  [Dxf, Dyf] = getNabla_o2p(mask, 'Forward', 'NeumannConstant');
  [Dxb, Dyb] = getNabla_o2p(mask, 'Backward', 'NeumannConstant');
end
